...
func toSQL(eq map[string]any, chain string) string {
	...
	for k, v := range eq {
		...
		if val, ok := v.(*Value); ok {
			...
			if val.isSkip || val.isNoZero && vv.IsZero() { // 排除该字段
				continue
			}
...
func inSQL(list reflect.Value) string {
	...
	for i := 0; i < list.Len(); i++ {
		...
		if val, ok := v.(*Value); ok {
			vv := reflect.ValueOf(val.value)
			if val.isSkip || val.isNoZero && vv.IsZero() { // 排除该元素
				continue
			}
		}
	...
	for k, v := range m {
		...
		if mv.Kind() == reflect.Slice || mv.Kind() == reflect.Array { // map 的值为切片或数组
			for i := mv.Len() - 1; i >= 0; i-- { // 正序删除 mv 元素会出错
				...
				if vv, ok := v.(*Value); ok {
					if vv.isSkip || vv.isNoZero && reflect.ValueOf(vv.value).IsZero() { // 排除该元素
		...
		if val, ok := v.(*Value); ok {
			if val.isSkip || val.isNoZero && reflect.ValueOf(val.value).IsZero() { // 排除该字段
				delete(m, k)
			}
		}
	}
